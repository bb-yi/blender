...
def sample_channel_mean(pixels, width, height, coordinates):
    values = [sample_pixel(pixels, width, height, x_ratio, y_ratio)[0] for x_ratio, y_ratio in coordinates]
...
def assert_plane_vs_sphere_curvature():
    ...
    sphere_mid_value = sample_channel_mean(
        sphere_pixels,
        sphere_width,
        sphere_height,
        [(0.7, 0.5), (0.5, 0.7), (0.3, 0.5), (0.5, 0.3)],
    )
    ...
    assert sphere_value > sphere_mid_value + 0.05, (
        f"Sphere curvature should peak toward the center, got center={sphere_center} mid_mean={sphere_mid_value}"
    )
    assert sphere_value > sphere_mid_value * 3.0, (
        f"Sphere curvature center should stay materially above the middle band, got center={sphere_center} mid_mean={sphere_mid_value}"
    )
...
def assert_rim_profile():
    ...
    assert plane_center[0] < 0.001, f"Plane rim center should stay near zero, got {plane_center}"
    ...
    assert edge[0] > center[0] * 4.0, f"Sphere rim edge should brighten, got center={center} edge={edge}"
