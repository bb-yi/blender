...
def setup_render():
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("LightShaderRegressionWorld")
...
def add_receiver():
    ...
    plane.name = "LightShaderRegressionReceiver"
...
def ensure_light_nodes(light):
    ...
    require(light.node_tree is not None, "light node tree was not created")
...
def add_scene_time_light():
    ...
    light_obj.name = "LightShaderRegressionPoint"
    light = light_obj.data
    light.name = "LightShaderRegressionPointData"
    light.energy = 80.0
    ...
    for y in range(height // 3, height * 2 // 3):
        for x in range(width // 3, width * 2 // 3):
            ...
            for c in range(3):
                sums[c] += pixels[i + c]
            count += 1
    require(count > 0, "center sample region was empty")
...
def main():
    ...
    size_1, pixels_1 = render_pixels(OUT_DIR / "scene_time_frame_001.png")
    color_1 = sample_center_rgb(size_1, pixels_1)

    bpy.context.scene.frame_set(11)
    size_11, pixels_11 = render_pixels(OUT_DIR / "scene_time_frame_011.png")
    color_11 = sample_center_rgb(size_11, pixels_11)
    diff = max_pixel_rgb_diff(size_1, pixels_1, size_11, pixels_11)

    require(
        color_11[0] > color_1[0] + 0.04,
        f"Scene Time red channel stayed stale: frame1={color_1}, frame11={color_11}",
    )
    require(
        color_1[2] > color_11[2] + 0.04,
        f"Scene Time blue channel stayed stale: frame1={color_1}, frame11={color_11}",
    )
    require(diff > 0.04, f"Scene Time render did not change enough: diff={diff}")

    blend_path = OUT_DIR / "light_shader_output_regression.blend"
    bpy.ops.wm.save_as_mainfile(filepath=str(blend_path))
    bpy.ops.wm.open_mainfile(filepath=str(blend_path))
    bpy.context.scene.frame_set(11)
    size_reload, pixels_reload = render_pixels(OUT_DIR / "reloaded_frame_011.png")
    ...
    print(
        "EEVEE_LIGHT_SHADER_OUTPUT_REGRESSION_RELEASE_OK "
