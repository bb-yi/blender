...
{
  const int2 probe_extent = int2(inst_.sphere_probes.probe_render_extent());
  const int2 max_extent = math::max(render_extent, probe_extent);
  /* Compute tile size and total word count. */
  const uint word_per_tile = divide_ceil_u(max_ii(lights_len_, 1), 32);
  int2 tiles_extent;
  ...
  do {
    ...
    const uint tile_count = tiles_extent.x * tiles_extent.y;
