...
#  define LIGHT_FOREACH_ALL_BEGIN( \
    ...
    { \
      ...
      uint word = 0u; \
      uint _l_index = (_culling.local_lights_len == 0u) ? 0xFFFFFFFFu : \
                                                           (_culling.local_lights_len - 1u); \
      ...
      while (true) { \
        ...
        if (state == LIGHT_FOREACH_STATE_LOCAL) { \
          ...
          word &= ~1u << uint(bit_index); \
          _l_index = word_idx * 32u + uint(bit_index); \
        } \
        if (state == LIGHT_FOREACH_STATE_DIRECTIONAL) { \
          _is_local = false; \
          _l_index = max(_culling.local_lights_len, _l_index + 1u); \
