...
{
  return drw_depth_screen_to_view(textureLod(hiz_tx, uv, 0.0f).r);
}
                                      ...
                                      out float scene_rim)
{
  ...
  for (int r = 0; r < 8; r++) {
    float angle = (float(r) + angle_offset) * 3.1415f * 0.25f * 0.5f;
    float2 screen_offset = curvature_rotate(float2(1.0f, 0.0f), angle) * screen_texel_size *
                           sample_radius * scale.xy;
    float2 hiz_offset = curvature_rotate(float2(1.0f, 0.0f), angle) * hiz_texel_size *
                        sample_radius * scale.xy;
