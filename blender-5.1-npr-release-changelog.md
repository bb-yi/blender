...
#### 新增功能
- `Outline Control` 新增真实 shell 描边方向的实验实现，用于让描边基于真实外壳几何而不是屏幕空间扩边结果。
- `Shell Width` 现在用于控制壳体几何沿法线方向的实际外扩距离，`Line Width` 继续负责描边宽度与检测宽度。

#### 修复与改进
- 壳体轮廓控制改由 `Shell Width` 和 shell 深度测试路径负责，避免将屏幕空间轮廓参数和真实 shell 路径混用。
- `Shell Width = 0` 时回退使用 `Line Width`，因此只设置线宽的材质会保持相同结果。
